...
export default function Jobs() {
  ...
  const postJob = async (e) => {
    e.preventDefault()
    if (!postForm.location || !postForm.salary) return toast.error(lang === 'te' ? 'అన్ని వివరాలు నింపండి' : 'Fill all required fields')
    try {
      ...
      toast.success(lang === 'te' ? 'ఉద్యోగం పోస్ట్ చేయబడింది!' : 'Job posted!')
      setTab('browse'); fetchJobs()
    } catch { toast.error(t('common.error')) }
  }

  const applyJob = async (job_id) => {
    if (!applyForm.worker_name || !applyForm.phone) return toast.error(lang === 'te' ? 'పేరు మరియు ఫోన్ నంబర్ ఎంటర్ చేయండి' : 'Enter your name and phone')
    try {
      await api.post('/api/apply_job', { job_id, ...applyForm })
      toast.success(lang === 'te' ? 'దరఖాస్తు సమర్పించబడింది!' : 'Application submitted!')
      setApplying(null); setApplyForm({ worker_name: '', phone: '' })
    } catch (err) { toast.error(err.response?.data?.detail || t('common.error')) }
  }

  const browseLabel = lang === 'te' ? 'ఉద్యోగాలు చూడండి' : 'Browse Jobs'
  const postLabel   = lang === 'te' ? 'ఉద్యోగం పోస్ట్ చేయండి' : 'Post a Job'
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '.6rem' }}>
          ...
          <div>
            <h1 style={{ fontSize: '1.4rem', fontWeight: 800, letterSpacing: '-.3px' }}>
              {lang === 'te' ? 'పొలం ఉద్యోగ మార్కెట్' : 'Farm Job Marketplace'}
            </h1>
            <p style={{ fontSize: '.83rem', color: 'var(--text-muted)', marginTop: '.1rem' }}>
              {lang === 'te' ? 'మీ సమీపంలో వ్యవసాయ పనులు వెతకండి లేదా పోస్ట్ చేయండి' : 'Find or post agricultural work near you'}
            </p>
          </div>
        </div>
        <div style={{ display: 'flex', background: 'var(--bg)', borderRadius: 10, padding: 3, border: '1px solid var(--border)' }}>
          {[{ key: 'browse', label: browseLabel }, { key: 'post', label: postLabel, icon: <Plus size={14} /> }].map(item => (
            <button key={item.key} onClick={() => setTab(item.key)}
              style={{
                ...
                background: tab === item.key ? 'var(--green)' : 'transparent',
                color: tab === item.key ? '#fff' : 'var(--text-muted)',
              }}>
        ...
        <>
            ...
            <div style={{ display: 'flex', gap: '.75rem' }}>
              <div style={{ position: 'relative', flex: 1 }}>
                ...
                <input value={location} onChange={e => setLocation(e.target.value)}
                  ...
                  placeholder={lang === 'te' ? 'స్థానం ద్వారా వెతకండి...' : 'Search by location...'} style={{ paddingLeft: '2.4rem' }} />
              </div>
              <button className="btn btn-primary" onClick={fetchJobs} disabled={loading}>
                {loading ? (lang === 'te' ? 'వెతుకుతున్నారు...' : 'Searching...') : (lang === 'te' ? 'వెతకండి' : 'Search')}
              </button>
            </div>
          </div>
          ...
          {!loading && jobs.length === 0 && (
            <div className="empty-state card">
              <Briefcase size={44} />
              <p>{lang === 'te' ? `${location} సమీపంలో ఉద్యోగాలు లేవు. వేరే స్థానం ప్రయత్నించండి.` : `No jobs found near ${location}. Try a different location.`}</p>
            </div>
          )}
            ...
            {jobs.map(job => (
              <div key={job.job_id} className="card" style={{ borderLeft: '4px solid var(--amber)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '.75rem' }}>
                  <div style={{ flex: 1 }}>
                    <h3 style={{ fontWeight: 700, fontSize: '1rem', marginBottom: '.5rem' }}>{job.crop_type} {lang === 'te' ? 'పొలం పని' : 'Farm Work'}</h3>
                    <div style={{ display: 'flex', gap: '.75rem', flexWrap: 'wrap' }}>
                      {[
                        { icon: <MapPin size={13} />, text: job.location },
                        { icon: <Clock size={13} />, text: job.working_hours },
                        { icon: <IndianRupee size={13} />, text: `₹${job.salary}/${lang === 'te' ? 'రోజు' : 'day'}` },
                        { icon: <Users size={13} />, text: `${job.workers_required} ${lang === 'te' ? 'కార్మికులు' : 'workers'}` },
                      ].map((item, i) => (
                  ...
                  <button className="btn btn-primary btn-sm" onClick={() => setApplying(applying === job.job_id ? null : job.job_id)}>
                    {applying === job.job_id ? (lang === 'te' ? 'రద్దు చేయండి' : 'Cancel') : (lang === 'te' ? 'దరఖాస్తు చేయండి' : 'Apply Now')}
                  </button>
                </div>
                ...
                {applying === job.job_id && (
                  <div className="fade-up" style={{ marginTop: '1rem', padding: '1rem', background: 'var(--green-pale)', borderRadius: 10, border: '1px solid #86efac' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '.75rem' }}>
                      <span style={{ fontWeight: 700, fontSize: '.9rem' }}>{lang === 'te' ? 'ఈ ఉద్యోగానికి దరఖాస్తు చేయండి' : 'Apply for this job'}</span>
                    ...
                    <div className="grid-2">
                      <div className="form-group" style={{ marginBottom: 0 }}>
                        <label>{lang === 'te' ? 'మీ పేరు' : 'Your Name'}</label>
                        <input value={applyForm.worker_name} onChange={e => setApplyForm(f => ({ ...f, worker_name: e.target.value }))} placeholder={lang === 'te' ? 'పూర్తి పేరు' : 'Full name'} />
                      </div>
                      <div className="form-group" style={{ marginBottom: 0 }}>
                        <label>{lang === 'te' ? 'ఫోన్ నంబర్' : 'Phone Number'}</label>
                        <input value={applyForm.phone} onChange={e => setApplyForm(f => ({ ...f, phone: e.target.value }))} placeholder={lang === 'te' ? 'మొబైల్ నంబర్' : 'Mobile number'} />
                      </div>
                    </div>
                    <button className="btn btn-primary btn-sm" style={{ marginTop: '.75rem' }} onClick={() => applyJob(job.job_id)}>
                      {lang === 'te' ? 'దరఖాస్తు సమర్పించండి' : 'Submit Application'}
                    </button>
                  </div>
                )}
              </div>
      ...
      {tab === 'post' && (
        <form className="card" onSubmit={postJob} style={{ maxWidth: 580 }}>
          <div className="section-title"><Plus size={16} color="var(--green)" /> {lang === 'te' ? 'పొలం ఉద్యోగం పోస్ట్ చేయండి' : 'Post a Farm Job'}</div>
          <div className="grid-2">
            <div className="form-group">
              <label>{lang === 'te' ? 'పంట రకం' : 'Crop Type'}</label>
              <select value={postForm.crop_type} onChange={e => setPostForm(f => ({ ...f, crop_type: e.target.value }))}>
                {CROPS.map(c => <option key={c}>{c}</option>)}
              </select>
            </div>
            <div className="form-group">
              <label>{lang === 'te' ? 'రోజువారీ జీతం (₹)' : 'Daily Salary (₹)'}</label>
            ...
            <div className="form-group">
              <label>{lang === 'te' ? 'పని గంటలు' : 'Working Hours'}</label>
              <input value={postForm.working_hours} onChange={e => setPostForm(f => ({ ...f, working_hours: e.target.value }))} placeholder="e.g. 6 AM - 3 PM" />
            </div>
            <div className="form-group">
              <label>{lang === 'te' ? 'అవసరమైన కార్మికులు' : 'Workers Required'}</label>
              <input type="number" min="1" value={postForm.workers_required} onChange={e => setPostForm(f => ({ ...f, workers_required: e.target.value }))} />
            </div>
          </div>
          <div className="form-group">
            <label>{lang === 'te' ? 'స్థానం' : 'Location'}</label>
            <input value={postForm.location} onChange={e => setPostForm(f => ({ ...f, location: e.target.value }))} placeholder={lang === 'te' ? 'గ్రామం / జిల్లా' : 'Village / District'} required />
          </div>
          <div className="form-group">
            <label>{lang === 'te' ? 'వివరణ (ఐచ్ఛికం)' : 'Description (optional)'}</label>
            <textarea rows={3} value={postForm.description} onChange={e => setPostForm(f => ({ ...f, description: e.target.value }))} placeholder={lang === 'te' ? 'అదనపు వివరాలు...' : 'Any additional details...'} />
          </div>
          <button type="submit" className="btn btn-primary" style={{ width: '100%', justifyContent: 'center' }}>
            <Briefcase size={16} /> {lang === 'te' ? 'ఉద్యోగం పోస్ట్ చేయండి' : 'Post Job'}
          </button>
        </form>
      )}
    </div>
